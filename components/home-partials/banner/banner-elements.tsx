...
export default function BannerElements({ banner }: { banner: BannerModel }) {
    return (
        <Link href={[3, 4].includes(banner.type) ? `/program/${banner.code}` : banner.url ?? "#"} className="flex flex-col justify-center gap-2 md:gap-4 lg:col-span-2 lg:-mt-8 lg:pb-15 md:pb-10 pb-4 fade-up pt-5 md:pt-0 relative">
            ...
            <div className="hidden mt-2 w-fit md:flex items-center gap-2 cursor-pointer lg:px-4 px-3 lg:py-2 md:py-1.5 py-1 rounded  border bg-white/10 hover:scale-105 transition text-[12px] font-semibold md:text-sm text-[#ffffff]">
                {banner?.type && [3, 4].includes(banner?.type) ? (
                    <>
